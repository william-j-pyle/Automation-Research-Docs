...
- [Button Card Usage Information](#button-card-usage-information)
  - [Features](#features)
...
![layout_image](assets/layout.png)
...
![css-var](assets/color-variable.gif)
...
![elements in the button](assets/button-card-elements.png)
...
![button card grid layout example with callouts](assets/button-card-grid-layout-example-with-callouts.png)
...
- Apple Homekit-like buttons:
  ...
  ![apple-like-buttons](assets/apple_style.gif)
...
![change_background](assets/loop_background.gif)
...
- Placing an element wherever you want (that means bypassing the grid). Set the grid to `position: relative` and set the element to `position: absolute`
  ...
  ![custom_fields_1](assets/custom_fields_1.gif)
...
- Or you can use the grid. Each element will have it's name positioned as the `grid-area`:
  ...
  ![custom_fields_2](assets/custom_fields_2.png)
...
- <a name="custom_fields_card_example"></a>Or you can embed a card (or multiple) inside the button card (note, this configuration uses [card-mod](https://github.com/thomasloven/lovelace-card-mod) to remove the `box-shadow` of the sensor card. This is what the `style` inside the embedded card is for):
  ...
  ![custom_fields_3](assets/custom_fields_card.png)
...
![blink-animation](assets/blink-animation.gif)
...
![label_template](assets/labels.png)
...
![per-style-config](assets/per-style-config.gif)
...
![lock-animation](assets/lock.gif)
...
![aspect-ratio-image](assets/aspect_ratio.png)
